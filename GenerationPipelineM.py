...
X_train_out = pad_sequences(train_out)
...
"""
i = Input(shape = (input_shape_in,))
i2 = Input(shape = input_shape_out)
combined = concatenate([i , i2])
x = Embedding(vocabulary + 1 , 10)(combined)
...
np.save('model_logs/TFChatbot_98Acc_Mem_history' , train.history)
model.save('models/TFChatbot_98Acc_Mem')
"""
...
while text != 'done':
    ...
    text_out = pad_sequences([text_out], 200)

    # Prediction
    pred = model.predict([text_in , text_out])
    pred = pred.argmax()
